...
export const associateSchema = z.object({
  ...
  phone: z
    ...
    .regex(
      /^\+?[0-9]{7,15}$/,
      "Use 7–15 digits, with an optional + prefix.",
    )
    .or(z.literal("")),
  ...
  z
    .object({
      amount: z
        .string()
        .trim()
        .min(1, "Enter a withdrawal amount.")
        .regex(
          /^\d+(?:\.\d{1,2})?$/,
    ...
    .superRefine((value, context) => {
      ...
      if (
        ...
        context.addIssue({
          code: "custom",
          path: ["amount"],
          message: `Minimum withdrawal is ${minimum} coins.`,
        });
      } else if (amount > maximum) {
        context.addIssue({
          code: "custom",
          path: ["amount"],
          message: `You can withdraw up to ${maximum} coins.`,
        });
      }

      if (
        ["one_time_upi", "upi_qr"].includes(
          value.destination,
        ) &&
        !/^[\w.-]+@[\w.-]+$/.test(value.upiId)
      ) {
        context.addIssue({
          code: "custom",
          path: ["upiId"],
      ...
      if (
        value.destination === "upi_qr" &&
        !value.payoutQrUrl
      ) {
        context.addIssue({
          code: "custom",
          path: ["payoutQrUrl"],
          message: "Upload a UPI QR image.",
        });
      }
    });
